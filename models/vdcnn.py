...
## Adapted from: https://github.com/dreamgonfly/deep-text-classification-pytorch/blob/master/models/VDCNN.py
# Paper: Alexis Conneau, Holger Schwenk, Loïc Barrault, Yann Lecun. 2017. Very Deep Convolutional Networks for Text Classification. arXiv:1606.01781.
...
class VDCNN(nn.Module):
    ...
    def __init__(self, depth=29, vocab_size=29, embedding_length=16, weights = {}, n_classes=3, k=2, shortcuts=True):
        super(VDCNN, self).__init__()

        vocabulary_size = vocab_size
        
        depth = depth
        embed_size = embedding_length # 16
        optional_shortcut = shortcuts # True
        k = k # 8
        ...
        if depth == 9:
            ...
            n_conv_layers = {'conv_block_512':4, 'conv_block_256':4,
                             'conv_block_128':4, 'conv_block_64':4}
        elif depth == 29:
            n_conv_layers = {'conv_block_512':4, 'conv_block_256':4,
        ...
        conv_layers = []
        conv_layers.append(nn.Conv1d(embedding_length, 64, kernel_size=3, padding=1))
